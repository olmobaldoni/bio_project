...
def run_inference(model_output_path, generated_images_dir, prompt, hyperparameters):
    ...
    for i in range(hyperparameters["num_generations"]):
        image = pipe(
            prompt, num_inference_steps=hyperparameters["num_inference_steps"]
        ).images[0]
...
def main(
    ...
    with open(train_log, "w") as f:
        f.write(f"Running LoRA training for the target images in {target_images_dir}.")
