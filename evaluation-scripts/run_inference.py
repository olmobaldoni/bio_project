...
def run_inference(
    ...
    subdirs_complex = {
        edit_prompt: os.path.join(subdir, f"{edit_prompt}")
        for edit_prompt in edit_prompts.keys()
    }
    ...
    for i in range(hyperparameters["num_generations"]):
        ...
        for edit_prompt in edit_prompts:
            image = pipe(
                edit_prompts[edit_prompt].replace("<placeholder>", placeholder_token),
                num_inference_steps=hyperparameters["num_inference_steps"],
                guidance_scale=hyperparameters["guidance_scale"],
            ).images[0]

            image.save(os.path.join(subdirs_complex[edit_prompt], f"image_{i}.png"))


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--model_output_path", type=str, required=False, default=DEFAULT_MODEL_NAME
    )
