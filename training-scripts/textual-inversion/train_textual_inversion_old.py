...
def main(
    ...
    #     model_output_dir = (
    #         f"../../fine-tuned-models/textual-inversion/{target_images_dir_name}"
    #     )
    # if generated_images_dir is None:
    #     generated_images_dir = (
    #         f"../../generated-images/textual-inversion/{target_images_dir_name}"
    #     )
    ...
    # se la folder dove vengono inserite le immagini generate esiste, questa viene eliminata
    os.system(f"rm -rf {generated_images_dir}")
    os.system(f"mkdir -p {generated_images_dir}")

    # se l'initializer token non viene assegnato, il nome della folder viene considerata il nuovo initializer token
...
if __name__ == "__main__":
    # target_images_dir: folder con le immagini di training --> è qui che sono contenute le immagini che rappresentano il concetto
    # log_file: log file di training
    # model_output_dir: folder con gli embeddings dei nuovi concetti appresi, qui vanno a finire gli embeddings corrispondenti al placeholder e.g., `<custom-style>` --> custom-style.bin
    # model_name: versione del modello di stable diffusion (ogni versione vuole dei parametri ad hoc, per ogni versione di stable-diffusion guarda il relativo readme.md)
    # placeholder_token: il nuovo token custom che viene appreso (<custom-style> token)
    # initializer_token: il token di partenza (per esempio se si vuole ottenere <custom-toy>, il token da cui iniziare potrebbe essere <toy>)
    # generated_images_dir: folder dove si trovano le immagini generate
    # no_train: flag per indicare se trainare o meno (il modello è fissato, gli embeddings vengono addestrati), se false si fa il train
    # resume_checkpoint: il training riparte dall'ultimo checkpoint disponibile
