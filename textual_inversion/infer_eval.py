...
# def get_parameter_combinations(base_embedding_dir: str):
#     num_inference_steps = config["hparams"]["num_inference_steps"]
#     guidance_scale = config["hparams"]["guidance_scale"]
#     num_generations = config["hparams"]["num_generations"]
...
#     subdirs = [
#         d
#         for d in os.listdir(base_embedding_dir)
#         if os.path.isdir(os.path.join(base_embedding_dir, d))
#     ]

#     file_sets = [
#         set(
#             f
#             for f in os.listdir(os.path.join(base_embedding_dir, d))
#             if f.endswith(".bin")
#         )
#         for d in subdirs
#     ]
#     common_embeddings = list(reduce(lambda x, y: x.intersection(y), file_sets))

#     for steps, scale, embedding_name in product(
#         num_inference_steps, guidance_scale, common_embeddings
#     ):
#         yield {
#             "num_inference_steps": steps,
#             "guidance_scale": scale,
...
#         results[gen_dir] = {"fid": fid_value, "inception_score": ins_value.item()}
...
# def run_inference(pipe, generated_images_dir: str, params: dict, emdedding_dir: str):
...
#     embedding_path = os.path.join(emdedding_dir, params["embedding_name"])

#     pipe.load_textual_inversion(
#         embedding_path, weight_name=params["embedding_name"], local_files_only=True
#     )
...
#     for params in get_parameter_combinations(base_embedding_dir):
...
#             run_inference(pipe, gen_dir_path, params, embedding_dir)
...
def run_inference(pipe, generated_images_dir: str, params: dict, embedding_dir: str):
    generated_images_dir_name = generated_images_dir.split("/")[-1]
    ...
    embedding_path = os.path.join(embedding_dir, params["embedding_name"])
...
def main():
    ...
    for params in get_parameter_combinations(base_embedding_dir):
        # Initialize the model once per parameter combination to avoid token conflicts
        for target_dir in os.listdir(target_images_dirs):
            embedding_dir = os.path.join(base_embedding_dir, target_dir)
            gen_dir_path = os.path.join(generated_images_dirs, target_dir)

            # Create a new pipeline for each target directory and parameter combination
            if torch.cuda.is_available():
                pipe = StableDiffusionPipeline.from_pretrained(
                    model_name,
                    torch_dtype=torch.float32,
