...
def compute_clip_cosine_similarity(target_images, generated_images):
    ...
    target_images = torch.stack([transform(image) for image in target_images]).to(
        device
    )
    generated_images = torch.stack([transform(image) for image in generated_images]).to(
        device
    )
    ...
    cosine_similarities = cosine_similarity(
        target_features.unsqueeze(1), generated_features.unsqueeze(0)
    )
...
def main(target_images_dir, generated_images_dir):
    ...
    avg_cosine_similarity = compute_clip_cosine_similarity(
        target_images, generated_images
    )
